...
function initialChart() {
    ...
    .then(d=>{
        console.log(d);
        if (d.tabs)  {
            let htmlTitle = `<li class="color-chart-tab active"><div class="color-chart-tab-title">${d.tabs[0].title}</div></li><li class="color-chart-tab"><div class="color-chart-tab-title">${d.tabs[1].title}</div></li>`;
            let html = `<div class="color-chart-tab-content active"><div><div class="color-chart-nav color-chart-nav-left"></div><div class="color-chart-slides">`;
            let htmlDots='';
            for (let i=0; i<d.tabs[0].img.length; i++) {
                let img = d.tabs[0].img[i];                    
                if (i==0) {
                    html+=`<div class="color-chart-slide active"><div class="color-chart-slide-item"> <img src="${img.link}" alt="${img.alt}" class="color-chart-slide-item-img"></div></div>`;
                    htmlDots+=`<div class="color-chart-dot active"></div>`;
                } else {
                    html+=`<div class="color-chart-slide"><div class="color-chart-slide-item"> <img src="${img.link}" alt="${img.alt}" class="color-chart-slide-item-img"></div></div>`;
            ...
            html+=`<div class="color-chart-tab-content"><div class="color-chart-video-wrapper"> <iframe class="color-chart-video" allowfullscreen src="${d.tabs[1].link}?start=0&amp;end=0&amp;autoplay=0&amp;loop=0&amp;rel=1" frameborder="0"> </iframe></div></div>`;
